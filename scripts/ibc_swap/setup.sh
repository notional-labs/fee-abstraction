...
osmosisd tx gamm create-pool --pool-file sample_pool.json --from validator1 --keyring-backend=test --home=$HOME/.osmosisd/validator1 --chain-id testing --yes
sleep 6
# get the pool id
POOL_ID=$(osmosisd query gamm pools -o json | jq -r '.pools[-1].id')

# Store the swaprouter contract
osmosisd tx wasm store scripts/bytecode/swaprouter.wasm --keyring-backend=test --home=$HOME/.osmosisd/validator1 --from deployer --chain-id testing --gas 10000000 --fees 25000stake --yes
# get the code id
sleep 6
...
osmosisd tx wasm instantiate $SWAPROUTER_CODE_ID "$INIT_SWAPROUTER" --keyring-backend=test --home=$HOME/.osmosisd/validator1 --from deployer --chain-id testing --label "test" --no-admin --yes 
sleep 5
...
osmosisd tx wasm execute "$SWAPROUTER_ADDRESS" "$CONFIG_SWAPROUTER" --keyring-backend=test --home=$HOME/.osmosisd/validator1 --from deployer --chain-id testing -y
sleep 5

# Store the crosschainswap contract
osmosisd tx wasm store scripts/bytecode/crosschain_swaps.wasm --keyring-backend=test --home=$HOME/.osmosisd/validator1 --from deployer --chain-id testing --gas 10000000 --fees 25000stake --yes
# get the code id
sleep 6
...
INIT_CROSSCHAIN_SWAPS='{"swap_contract":"'$SWAPROUTER_ADDRESS'","channels":[["osmo", "'$CHANNEL_ID'"]]}'
osmosisd tx wasm instantiate $CROSSCHAIN_SWAPS_CODE_ID "$INIT_CROSSCHAIN_SWAPS" --keyring-backend=test --home=$HOME/.osmosisd/validator1 --from deployer --chain-id testing --label "test" --no-admin --yes 
sleep 5
...
feeappd tx ibc-transfer transfer transfer $CHANNEL_ID $CROSSCHAIN_SWAPS_ADDRESS 100uosmo \
    --from validator -y \
    --note "$MEMO"
